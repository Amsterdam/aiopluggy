...
are managed by an instance of a :py:class:`aiopluggy.PluginManager` which
defines the primary ``aiopluggy`` API.

In order for a ``PluginManager`` to detect functions in a namespace
intended to be *hooks*, they must be decorated using special ``aiopluggy`` *marks*.

.. _marking_hooks:

Marking hooks
-------------
The :py:class:`~aiopluggy.HookspecMarker` and :py:class:`~aiopluggy.HookimplMarker`
decorators are used to *mark* functions for detection by a ``PluginManager``:

.. code-block:: python

    from aiopluggy import HookspecMarker, HookimplMarker
...
:py:meth:`~aiopluggy.PluginManager.register()` method:
...
The generator is `sent`_ a :py:class:`aiopluggy.Result` object which can
...
:py:meth:`~aiopluggy.Result.force_result` or
:py:meth:`~aiopluggy.Result.get_result` methods.

.. note::
    Hook wrappers can **not** return results (as per generator function
    semantics); they can only modify them using the ``Result`` API.
...
:py:meth:`~aiopluggy.PluginManager.add_hookspecs()` method and normally
should be added before registering corresponding *hookimpls*:

.. code-block:: python

    import sys
    from aiopluggy import PluginManager, HookspecMarker
...
:py:meth:`~aiopluggy.PluginManager.check_pending()` method. If you'd like
...
can be called with :py:meth:`~aiopluggy.PluginManager.call_historic()` **before**
...
:py:class:`aiopluggy.PluginManager`.
...
:py:meth:`~aiopluggy.PluginManager.register()` and
:py:meth:`~aiopluggy.PluginManager.add_hookspec()` methods to collect
hook *implementations* and *specfications* from *plugin* namespaces respectively.

You can unregister any *plugin*'s hooks using
:py:meth:`~aiopluggy.PluginManager.unregister()` and check if a plugin is
registered by passing its name to the
:py:meth:`~aiopluggy.PluginManager.is_registered()` method.

Loading ``setuptools`` entry points
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
You can automatically load plugins registered through `setuptools entry points`_
with the :py:meth:`~aiopluggy.PluginManager.load_setuptools_entrypoints()`
method.
...
:py:meth:`~aiopluggy.PluginManager.set_blocked()` and check if a given
*plugin* is blocked by name using :py:meth:`~aiopluggy.PluginManager.is_blocked()`.
...
- :py:meth:`~aiopluggy.PluginManager.list_name_plugin()` -
  return a list of name-plugin pairs
- :py:meth:`~aiopluggy.PluginManager.get_plugins()` - retrieve all plugins
- :py:meth:`~aiopluggy.PluginManager.get_canonical_name()`- get a *plugin*'s
  canonical name (the name it was registered with)
- :py:meth:`~aiopluggy.PluginManager.get_plugin()` - retrieve a plugin by its
  canonical name
...
:py:meth:`~aiopluggy.PluginManager._parse_hookspec_opts()` and
:py:meth:`~aiopluggy.PluginManager.get_hookimpl_flags()` respectively.
...
a :py:class:`aiopluggy.HookCaller` which in turn *loops* through the
``1:N`` registered *hookimpls* and calls them in sequence.

Every :py:class:`aiopluggy.PluginManager` has a ``hook`` attribute
which is an instance of this :py:class:`aiopluggy._HookRelay`.
...
using the :py:meth:`aiopluggy.HookCaller.call_historic()` method:
...
Note that if you ``call_historic()`` the ``HookCaller`` (and thus your
...
:py:meth:`aiopluggy.HookCaller.call_extra()` method.


Calling with a subset of registered plugins
-------------------------------------------
You can make a call using a subset of plugins by asking the
``PluginManager`` first for a ``HookCaller`` with those plugins removed
using the :py:meth:`aiopluggy.PluginManager.subset_hook_caller()` method.

You then can use that ``HookCaller`` to make normal, ``call_historic()``,
...
Built-in tracing
****************
``aiopluggy`` comes with some batteries included hook tracing for your
debugging needs.
...
:py:meth:`aiopluggy.PluginManager.enable_tracing()` method which returns an
...
:py:class:`aiopluggy.PluginManager.add_hookcall_monitoring()`.
...
Development
***********
Great care must taken when hacking on ``aiopluggy`` since multiple mature
...
Major releases (incremented X in "X.Y.Z").  If you want to use ``aiopluggy``
in your project you should thus use a dependency restriction like
``"aiopluggy>=0.1.0,<1.0"`` to avoid surprises.
...
.. _tox test suite:
    https://github.com/pytest-dev/aiopluggy/blob/master/tox.ini
.. _Semantic Versioning:
    http://semver.org/
.. _tight coupling:
    https://en.wikipedia.org/wiki/Coupling_%28computer_programming%29#Types_of_coupling
.. _Python interpreters:
    https://github.com/pytest-dev/aiopluggy/blob/master/tox.ini#L2
.. _200+ plugins:
